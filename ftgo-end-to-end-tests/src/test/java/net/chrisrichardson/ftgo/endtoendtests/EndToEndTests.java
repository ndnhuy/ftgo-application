...
public class EndToEndTests {
      ...
      RestAssuredConfig
        ...
        .objectMapperConfig(
          new ObjectMapperConfig()
            .jackson2ObjectMapperFactory((aClass, s) -> JSonMapper.objectMapper)
        );
  }
      ...
      () -> {
        ...
        assertEquals(
          priceOfChickenVindaloo
            .multiply(revisedQuantityOfChickenVindaloo)
            .asString(),
          orderTotal
        );
      }
  ...
  private void reviseOrder(int orderId) {
    given()
      .body(
        new ReviseOrderRequest(
          Collections.singletonList(
            new RevisedOrderLineItem(
  ...
  private int createRestaurant() {
    CreateRestaurantRequest request = new CreateRestaurantRequest()
      ...
      .address(
        new net.chrisrichardson.ftgo.apis.model.restaurantservice.Address()
      ...
      .menu(
        new RestaurantMenu()
          .addMenuItemsItem(
            new MenuItem()
              .id(CHICKED_VINDALOO_MENU_ITEM_ID)
  ...
  private void verifyRestaurantCreatedInKitchenService(int restaurantId) {
    Eventually.eventually(
      String.format("verifyRestaurantCreatedInKitchenService %s", restaurantId),
      () ->
        given()
          .when()
  ...
  private void verifyRestaurantCreatedInOrderService(int restaurantId) {
    Eventually.eventually(
      String.format("verifyRestaurantCreatedInOrderService %s", restaurantId),
      () ->
        given()
          .when()
    ...
    Integer orderId = given()
      .body(
        new CreateOrderRequest(
          ...
          LocalDateTime.now(),
          Collections.singletonList(
            new CreateOrderRequest.LineItem(CHICKED_VINDALOO_MENU_ITEM_ID, 5)
          )
        )
      )
      .contentType("application/json")
  ...
  private void verifyOrderHistoryUpdated(
    int orderId,
    int consumerId,
  ...
  private void noteCourierAvailable() {
    ...
    given()
      ...
      .when()
      .post(
        deliveryServiceBaseUrl(
          "couriers",
          Long.toString(courierId),
          "availability"
        )
      )
      .then()
